...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $submitted_code = '';
    // Concatenate the 6 digits from the form
    for ($i = 1; $i <= 6; $i++) {
        $submitted_code .= $_POST["digit$i"];
    }
    
    if (!isset($_SESSION['verification_code']) || 
        !isset($_SESSION['verification_email']) || 
        !isset($_SESSION['code_expiry'])) {
        $response['message'] = 'Verification session expired. Please try again.';
    ...
    if ($submitted_code == $_SESSION['verification_code']) {
        // Code is correct, create user session
        $_SESSION['user_email'] = $_SESSION['verification_email'];
...
<body>
    <div class="container">
        ...
        <form action="Auth-Verify-Code.php" method="POST">
