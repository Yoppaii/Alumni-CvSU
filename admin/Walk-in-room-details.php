...
$book_rooms = [
   ['id' => 1, 'name' => 'Room 1', 'max_occupancy' => 4,
   ...
   ['id' => 2, 'name' => 'Room 2', 'max_occupancy' => 4,
    ...
    'amenities' => ['Free Wi-Fi', 'Smart TV', 'Air Conditioning', 'Coffee Maker', 'Private Bathroom', 'Room Safe', 'Mini Fridge', 'Work Desk']],
   ['id' => 3, 'name' => 'Room 3', 'max_occupancy' => 2,
   ...
   ['id' => 6, 'name' => 'Room 6', 'max_occupancy' => 4,
    ...
    'amenities' => ['Free Wi-Fi', 'Smart TV', 'Air Conditioning', 'Coffee Maker', 'Private Bathroom', 'Room Safe', 'Mini Fridge', 'Work Desk']],
   ['id' => 7, 'name' => 'Room 7', 'max_occupancy' => 3,
   ...
   ['id' => 10, 'name' => 'Conference Room', 'max_occupancy' => 50,
    ...
    ['id' => 11, 'name' => 'Lobby', 'max_occupancy' => 100,
   ...
   ['id' => 12, 'name' => 'Building', 'max_occupancy' => 500,
...
$pricing_table = 'room_price'; 
if ($room['name'] === 'Lobby') {
   $pricing_table = 'lobby_pricing';
} elseif ($room['name'] === 'Conference Room') {
   $pricing_table = 'conference_pricing';
} elseif ($room['name'] === 'Board Room') {
   $pricing_table = 'board_pricing';
} elseif ($room['name'] === 'Building') {
   $pricing_table = 'building_pricing';
}
...
<body class="room-details-body">
              ...
              <div class="room-details-grid">
                      ...
                      <div class="room-details-amenities">
                          <?php foreach ($room['amenities'] as $amenity): ?>
                              <div class="room-details-amenity">
                                  ...
                                  $icon = 'check';
                                  switch ($amenity) {
                                      case 'Free Wi-Fi': $icon = 'wifi'; break;
                                      case 'TV': $icon = 'tv'; break;
                                      case 'Air Conditioning': $icon = 'snowflake'; break;
                                      case 'Coffee Maker': $icon = 'coffee'; break;
                                      case 'Private Bathroom': $icon = 'bath'; break;
                                      case 'Room Safe': $icon = 'shield-alt'; break;
                                      case 'Mini Fridge': $icon = 'box'; break;
                                      case 'Work Desk': $icon = 'desk'; break;
                                      case 'Kitchen': $icon = 'utensils'; break;
                                      case 'Lounge Area': $icon = 'couch'; break;
                                  }
                                  ?>
                  ...
                  <div class="room-details-sidebar">
                      <div class="room-details-guest-selection">
                          <label for="room-details-guest-count" style="display: block; margin-bottom: 8px; color: #4b5563; font-weight: 500;">
                              Number of Guests
                          </label>
                          <select id="room-details-guest-count" class="room-details-guest-select">
                              <option value="">Select number of guests</option>
                              <?php foreach ($occupancies as $occupancy): ?>
                                  <option value="<?php echo $occupancy['occupancy']; ?>" 
                                          data-price="<?php echo $occupancy['price']; ?>">
                                      <?php echo $occupancy['occupancy']; ?> 
                                      <?php echo $occupancy['occupancy'] === 1 ? 'Guest' : 'Guests'; ?>
                                  </option>
                              <?php endforeach; ?>
                          </select>
                          <div id="room-details-price-display" class="room-details-price-display"></div>
                      </div>
  ...
  <script>
     document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('room-details-guest-count').addEventListener('change', function() {
            ...
            if (price) {
                priceDisplay.textContent = `Price: ₱${price} per day`;  
                priceDisplay.style.display = 'block';
                bookButton.disabled = false;

                sessionStorage.setItem('selectedPrice', price);
                sessionStorage.setItem('selectedGuests', select);
            } else {
                priceDisplay.textContent = 'Error: Price not available';
                priceDisplay.style.display = 'block';
                bookButton.disabled = true;
            }
        });

        document.getElementById('room-details-book-button').addEventListener('click', function() {
            const guests = document.getElementById('room-details-guest-count').value;
            const selectedOption = document.getElementById('room-details-guest-count').options[document.getElementById('room-details-guest-count').selectedIndex];
            const price = selectedOption.getAttribute('data-price');
