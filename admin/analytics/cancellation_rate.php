...
$query = "SELECT 
            ...
            SUM(CASE WHEN b.status IN ('cancelled', 'no-show') THEN 1 ELSE 0 END) AS cancelled,
            SUM(CASE WHEN b.status = 'completed' THEN 1 ELSE 0 END) AS successful,
            (SUM(CASE WHEN b.status IN ('cancelled', 'no-show') THEN 1 ELSE 0 END) * 100 / COUNT(*)) AS rate
          FROM bookings b
...
if ($result) {
    ...
    echo json_encode(["rate" => $rate, "total_bookings" => $total_bookings, "cancelled" => $cancelled, "successful" => $successful]);
} else {
    echo json_encode(["rate" => 0, "cancelled" => 0, "successful" => 0]);
}
