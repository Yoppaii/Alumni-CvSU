...
// Update the SQL query to group by degree_specialization instead of civil_status
$query = "SELECT degree_specialization, COUNT(*) as count 
...
while ($row = $result->fetch_assoc()) {
    $labels[] = $row['degree_specialization']; // Change to degree_specialization
    $data[] = $row['count'];
    $total += $row['count'];
}
?>
...
<div class="education-background-distribution-analytics">

    <div class="analytics-header">
        <a href="?section=charts" class="analytics-back-btn">
            <i class="fas fa-arrow-left"></i> Back to Survey Results
        </a>
        <h2 class="analytics-title">Educational Background Distribution Analysis</h2> <!-- Change title -->
    </div>
    ...
    <div class="analytics-content">
        <div class="analytics-chart-container">
            <canvas id="educationBackgroundDistribution"></canvas> <!-- Change canvas id -->
        </div>
...
<style>
    .education-background-distribution-analytics {
        width: 100%;
        max-width: 1200px;
        margin: 0 auto;
        padding: 20px;
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function createChart(type) {
            const ctx = document.getElementById('educationBackgroundDistribution').getContext('2d'); // Change canvas id
            ...
            const config = {
                type: type,
                data: {
                    labels: <?php echo json_encode($labels); ?>, // Degree specializations
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        title: {
                            display: true,
                            text: 'Distribution by Degree Specialization', // Change title
                            font: {
                                size: 18
                            },
                            color: getComputedStyle(document.documentElement).getPropertyValue('--text-primary')
