<?php
//view-civil-status.php
require_once 'main_db.php';

// Update the SQL query to group by civil_status instead of other fields
$query = "SELECT civil_status, COUNT(*) as count 
          FROM personal_info 
...
while ($row = $result->fetch_assoc()) {
    $labels[] = $row['civil_status']; // Change to civil_status
    $data[] = $row['count'];
    $total += $row['count'];
}
?>
...
<div class="civil-status-distribution-analytics">

    <div class="analytics-header">
        <a href="?section=charts" class="analytics-back-btn">
            <i class="fas fa-arrow-left"></i> Back to Survey Results
        </a>
        <h2 class="analytics-title">Civil Status Distribution Analysis</h2> <!-- Change title -->
    </div>
    ...
    <div class="analytics-content">
        <div class="analytics-chart-container">
            <canvas id="civilStatusDistribution"></canvas> <!-- Change canvas id -->
        </div>
...
<style>
    .civil-status-distribution-analytics {
        width: 100%;
        max-width: 1200px;
        margin: 0 auto;
        padding: 20px;
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function createChart(type) {
            const ctx = document.getElementById('civilStatusDistribution').getContext('2d'); // Change canvas id
            ...
            const config = {
                type: type,
                data: {
                    labels: <?php echo json_encode($labels); ?>, // Civil status names
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        title: {
                            display: true,
                            text: 'Distribution by Civil Status', // Change title
                            font: {
                                size: 18
                            },
                            color: getComputedStyle(document.documentElement).getPropertyValue('--text-primary')
