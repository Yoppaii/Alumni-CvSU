...
// Update the SQL query to group by residence instead of campus
$query = "SELECT residence, COUNT(*) as count 
          FROM personal_info 
          GROUP BY residence 
          ORDER BY count DESC";
...
while ($row = $result->fetch_assoc()) {
    $labels[] = $row['residence']; // Change to residence
    $data[] = $row['count'];
    $total += $row['count'];
}
?>
...
<div class="residence-distribution-analytics">

    <div class="analytics-header">
        <a href="?section=charts" class="analytics-back-btn">
            <i class="fas fa-arrow-left"></i> Back to Survey Results
        </a>
        <h2 class="analytics-title">Residence Distribution Analysis</h2> <!-- Change title -->
    </div>
    ...
    <div class="analytics-content">
        <div class="analytics-chart-container">
            <canvas id="residenceDistribution"></canvas> <!-- Change canvas id -->
        </div>
...
<style>
    .residence-distribution-analytics {
        width: 100%;
        max-width: 1200px;
        margin: 0 auto;
        padding: 20px;
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function createChart(type) {
            const ctx = document.getElementById('residenceDistribution').getContext('2d'); // Change canvas id
            ...
            const config = {
                type: type,
                data: {
                    labels: <?php echo json_encode($labels); ?>, // Residence names
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        title: {
                            display: true,
                            text: 'Distribution by Residence', // Change title
                            font: {
                                size: 18
                            },
                            color: getComputedStyle(document.documentElement).getPropertyValue('--text-primary')
