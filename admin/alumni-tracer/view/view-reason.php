<?php
// view-reason-for-taking.php
require_once 'main_db.php';

// Change the SQL query to group by reason_for_taking
$query = "SELECT reason_for_taking, COUNT(*) as count 
          FROM educational_background 
          GROUP BY reason_for_taking 
          ORDER BY count DESC";
...
while ($row = $result->fetch_assoc()) {
    $labels[] = $row['reason_for_taking']; // Change to reason_for_taking
    $data[] = $row['count'];
    $total += $row['count'];
}
?>
...
<div class="reason-for-taking-distribution-analytics">

    <div class="analytics-header">
        <a href="?section=charts" class="analytics-back-btn">
            <i class="fas fa-arrow-left"></i> Back to Survey Results
        </a>
        <h2 class="analytics-title">Reason for Taking Analysis</h2> <!-- Change title -->
    </div>
    ...
    <div class="analytics-content">
        <div class="analytics-chart-container">
            <canvas id="reasonForTakingDistribution"></canvas> <!-- Change canvas id -->
        </div>
...
<style>
    .reason-for-taking-distribution-analytics {
        width: 100%;
        max-width: 1200px;
        margin: 0 auto;
        padding: 20px;
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function createChart(type) {
            const ctx = document.getElementById('reasonForTakingDistribution').getContext('2d'); // Change canvas id
            ...
            const config = {
                type: type,
                data: {
                    labels: <?php echo json_encode($labels); ?>, // Reason for taking
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        title: {
                            display: true,
                            text: 'Distribution by Reason for Taking', // Change title
                            font: {
                                size: 18
                            },
                            color: getComputedStyle(document.documentElement).getPropertyValue('--text-primary')
