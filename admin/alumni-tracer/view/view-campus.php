...
// Change the SQL query to group by campus
$query = "SELECT campus, COUNT(*) as count 
          FROM personal_info 
          GROUP BY campus 
          ORDER BY count DESC";
...
while ($row = $result->fetch_assoc()) {
    $labels[] = $row['campus']; // Change to campus
    $data[] = $row['count'];
    $total += $row['count'];
}
?>
...
<div class="campus-distribution-analytics">

    <div class="analytics-header">
        <a href="?section=charts" class="analytics-back-btn">
            <i class="fas fa-arrow-left"></i> Back to Survey Results
        </a>
        <h2 class="analytics-title">Campus Distribution Analysis</h2> <!-- Change title -->
    </div>
    ...
    <div class="analytics-content">
        <div class="analytics-chart-container">
            <canvas id="campusDistribution"></canvas> <!-- Change canvas id -->
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function createChart(type) {
            const ctx = document.getElementById('campusDistribution').getContext('2d'); // Change canvas id
            ...
            const config = {
                type: type,
                data: {
                    labels: <?php echo json_encode($labels); ?>, // Campus names
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        title: {
                            display: true,
                            text: 'Distribution by Campus', // Change title
                            font: {
                                size: 18
                            },
                            color: getComputedStyle(document.documentElement).getPropertyValue('--text-primary')
