<?php
// view-highest-education.php
require_once 'main_db.php';

// Change the SQL query to group by highest_education
$query = "SELECT highest_education, COUNT(*) as count 
...
while ($row = $result->fetch_assoc()) {
    $labels[] = $row['highest_education']; // Change to highest_education
    $data[] = $row['count'];
    $total += $row['count'];
}
?>
...
<div class="highest-education-distribution-analytics">

    <div class="analytics-header">
        <a href="?section=charts" class="analytics-back-btn">
            <i class="fas fa-arrow-left"></i> Back to Survey Results
        </a>
        <h2 class="analytics-title">Highest Education Level Distribution Analysis</h2> <!-- Change title -->
    </div>
    ...
    <div class="analytics-content">
        <div class="analytics-chart-container">
            <canvas id="highestEducationDistribution"></canvas> <!-- Change canvas id -->
        </div>
...
<style>
    .highest-education-distribution-analytics {
        width: 100%;
        max-width: 1200px;
        margin: 0 auto;
        padding: 20px;
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function createChart(type) {
            const ctx = document.getElementById('highestEducationDistribution').getContext('2d'); // Change canvas id
            ...
            const config = {
                type: type,
                data: {
                    labels: <?php echo json_encode($labels); ?>, // Highest education levels
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        title: {
                            display: true,
                            text: 'Distribution by Highest Education Level', // Change title
                            font: {
                                size: 18
                            },
                            color: getComputedStyle(document.documentElement).getPropertyValue('--text-primary')
