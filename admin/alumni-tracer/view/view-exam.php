<?php
// view-professional-exams.php
require_once 'main_db.php';

// Change the SQL query to group by professional_exams
$query = "SELECT professional_exams, COUNT(*) as count 
          FROM educational_background 
          GROUP BY professional_exams 
          ORDER BY count DESC";
...
while ($row = $result->fetch_assoc()) {
    $labels[] = $row['professional_exams']; // Change to professional_exams
    $data[] = $row['count'];
    $total += $row['count'];
}
?>
...
<div class="professional-exams-distribution-analytics">

    <div class="analytics-header">
        <a href="?section=charts" class="analytics-back-btn">
            <i class="fas fa-arrow-left"></i> Back to Survey Results
        </a>
        <h2 class="analytics-title">Professional Exams Distribution Analysis</h2> <!-- Change title -->
    </div>
    ...
    <div class="analytics-content">
        <div class="analytics-chart-container">
            <canvas id="professionalExamsDistribution"></canvas> <!-- Change canvas id -->
        </div>
...
<style>
    .professional-exams-distribution-analytics {
        width: 100%;
        max-width: 1200px;
        margin: 0 auto;
        padding: 20px;
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function createChart(type) {
            const ctx = document.getElementById('professionalExamsDistribution').getContext('2d'); // Change canvas id
            ...
            const config = {
                type: type,
                data: {
                    labels: <?php echo json_encode($labels); ?>, // Professional exams names
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        title: {
                            display: true,
                            text: 'Distribution by Professional Exams', // Change title
                            font: {
                                size: 18
                            },
                            color: getComputedStyle(document.documentElement).getPropertyValue('--text-primary')
